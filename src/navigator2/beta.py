...
class BetaMixin:
    def __init__(
        ...
        self._b_actions_min = torch.tensor(
            self.action_space.low, device=self.device, dtype=torch.float32
        )
        self._b_actions_max = torch.tensor(
    ...
    def act(
        ...
        if taken_actions is not None:
            taken_actions = (taken_actions - self._b_actions_min) / (
                self._b_actions_max - self._b_actions_min
            )
        else:
