...
class DualHeadUNet3D(nn.Module):
    """
    Dual head version of UNet3D
    """
    def __init__(self, in_channels, out_channels, final_sigmoid=True, f_maps=64, layer_order='gcr',
                 num_groups=8, num_levels=4, is_segmentation=True, conv_padding=1,
                 conv_upscale=2, upsample='default', dropout_prob=0.1, conv_kernel_size=3, pool_kernel_size=2,
                 basic_module=DoubleConv, is3d=True, dual_head=True, **kwargs):
        super().__init__()
        ...
        self.decoders_seg = create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding,
                                               layer_order, num_groups, upsample, dropout_prob,
                                               is3d)

        if self.dual_head:
            self.decoders_sdf = create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding,
                                                   layer_order, num_groups, upsample, dropout_prob,
                                                   is3d)
        ...
        # Return raw model output, no logits
        # if is_segmentation:
        #     # semantic segmentation problem
        #     if final_sigmoid:
        #         self.final_activation = nn.Sigmoid()
    ...
    def do_decoder_path(self, decoders, encoders_features, x):
        for i, (decoder, encoder_features) in enumerate(zip(decoders, encoders_features)):
            # pass the output from the corresponding encoder and the output
            # of the previous decoder
            x = decoder(encoder_features, x)
    ...
    def decoder_seg(self, encoders_features, x):
        x = self.do_decoder_path(self.decoders_seg, encoders_features, x)
    ...
    def decoder_sdf(self, encoders_features, x):
        x_sdf = self.do_decoder_path(self.decoders_sdf, encoders_features, x)
        return x_sdf

    @staticmethod
    def combine_output(x_seg, x_sdf):
        return torch.cat([x_seg, x_sdf], dim=1)
    ...
    def forward(self, x):
        ...
        # Decoders
        x_seg = self.decoder_seg(encoders_features, x)
        if self.dual_head:
            x_sdf = self.decoder_sdf(encoders_features, x_sdf)
        else:
            x_sdf = torch.zeros_like(x_seg)

        # Combine
        x = self.combine_output(x_seg, x_sdf)
        return x
