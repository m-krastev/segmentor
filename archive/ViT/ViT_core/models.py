...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size):
        ...
        self.hybrid_model = CNNEncoder(config, n_channels=1)
    ...
    def forward(self, x):
        ...
        x = self.patch_embeddings(x)  # (hidden, n_patches^(1/3), n_patches^(1/3), n_patches^(1/3))  TODO: waar is batch gebleven?
        
        x = x.flatten(1)  # (B, hidden, n_patches) TODO: batch is loesoe
        x = x.transpose(-1, -2)  # (B, n_patches , hidden) TODO: batch is loesoe
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        if out_size is None:
            ...
            self.up = nn.Upsample(size=out_size, mode='trilinear', align_corners=False)


    def forward(self, x, skip=None):
        print("INP", x.shape, "Skip", skip.shape)
        x = self.up(x)
        if skip is not None:
...
class DecoderCup(nn.Module):
    def __init__(self, config, img_size):
        ...
        out_sizes = ((20, 20, 20), (40, 40, 40), (80, 80, 80), (100, 160, 160))
        blocks = [
            DecoderBlock(in_ch, out_ch, sk_ch, out_size) for in_ch, out_ch, sk_ch, out_size in zip(in_channels, out_channels, skip_channels, out_sizes)
        ]
...
class ViTSeg(nn.Module):
    def __init__(self, config, img_size=(64, 256, 256), vis=False):
        super().__init__()
        self.transformer = Transformer(config, img_size, vis)
        self.decoder = DecoderCup(config, img_size)
        self.reg_head = RegistrationHead(
