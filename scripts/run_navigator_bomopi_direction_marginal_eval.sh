...
COMMON_ARGS=(
  ...
  --selection mode
  --categorical-deterministic-decoding direction_marginal_mode
)
...
uv run --no-sync python scripts/evaluate_navigator_stochastic.py \
  ...
  --output-dir "${RESULT_ROOT}/gru-filters-development64k-v1-direction-marginal-best" \
  "${COMMON_ARGS[@]}"
...
uv run --no-sync python scripts/evaluate_navigator_stochastic.py \
  ...
  --output-dir "${RESULT_ROOT}/gru-filters-pretrained-development64k-v1-direction-marginal-best" \
  "${COMMON_ARGS[@]}"
